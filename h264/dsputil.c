...
#ifndef av_bswap32
static av_always_inline av_const uint32_t av_bswap32(uint32_t x) //add lb--11-03-03
{
...
static void put_pixels_nonclamped_c(const DCTELEM *block, uint8_t *pixels,
                                    int line_size)
{
    int i;
...
inline av_const int mid_pred(int a, int b, int c)//add lb--11-03-05
{
#if 0
    int t= (a-b)&((a-b)>>31);
    a-=t;
...
//    ff_simple_idct(temp); //FIXME   //del lb--11-03-05
...
/*    if(avctx->lowres==1){
        ...
        }else if((CONFIG_VP3_DECODER || CONFIG_VP5_DECODER || CONFIG_VP6_DECODER ) &&  //del lb--11-03-03
...
*/
	c->idct_permutation_type= FF_NO_IDCT_PERM;  //add lb--11-03-05
